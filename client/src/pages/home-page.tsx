...
export default function HomePage() {
  ...
  // Fetch mortgages
  const { data: mortgages, isLoading: isMortgagesLoading } = useQuery<Mortgage[]>({
  ...
  const updateMortgageMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number, data: Partial<Mortgage> }) => {
  ...
  const updateScenarioMutation = useMutation({
    ...
    onSuccess: () => {
      if (selectedMortgageId) {
        queryClient.invalidateQueries({ queryKey: ["/api/mortgages", selectedMortgageId, "scenarios"] });
      }
    },
    onError: (error: Error) => {
      toast({
  ...
  const createScenarioMutation = useMutation({
    mutationFn: async ({ mortgageId, data }: { mortgageId: number, data: Omit<Scenario, 'id' | 'mortgageId'> }) => {
      const res = await apiRequest("POST", `/api/mortgages/${mortgageId}/scenarios`, data);
      return res.json();
    },
    onSuccess: () => {
      if (selectedMortgageId) {
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-grow flex items-center justify-center">
          <div className="text-center">
            ...
            <h2 className="text-xl font-medium">Loading your mortgage data...</h2>
          </div>
        </main>
        <Footer />
      </div>
        ...
        <main className="flex-grow">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div className="text-center mb-10">
              ...
              <h2 className="text-2xl font-semibold mb-2">No Mortgages Found</h2>
              <p className="text-neutral-600 mb-6">
                Get started by adding your first mortgage details to see how you can save with early payments.
              </p>
              <Button 
                size="lg"
                onClick={() => {
